...
-record(state, {
    ...
    %% 0 means no topic aliases allowed.
    topic_alias_max :: non_neg_integer(),
    %% 0 means no topic aliases allowed.
    topic_alias_max_out :: non_neg_integer(),
    %% topic aliases used from client to broker.
    topic_aliases_in = #{} :: topic_aliases_in(),
    %% topic aliases used from broker to client.
    topic_aliases_out = #{} :: topic_aliases_out(),
...
init(
    ...
    TopicAliasMaxOut = maybe_get_topic_alias_max(
        Properties, vmq_config:get_env(topic_alias_max_broker, 0)
    ),
    ...
    case lists:member(ProtoVer, AllowedProtocolVersions) of
        true ->
            case check_enhanced_auth(ConnectFrame, State) of
                {stop, _, _} = R -> R;
                {pre_connect_auth, NewState, Out} -> {{pre_connect_auth, NewState}, Out};
...
maybe_strip_problem_information(F, _, _, true) ->
    F;
maybe_strip_problem_information(F, _, undefined, false) ->
    F;
...
maybe_reduce_packet_size(F0, _, undefined) ->
    vmq_parser_mqtt5:serialise(F0);
...
pre_connect_auth(
    #mqtt5_auth{
        properties =
            #{
                p_authentication_method := AuthMethod,
                p_authentication_data := _
            } = Props,
        reason_code = RC
    },
    #state{
        enhanced_auth = #auth_data{
            method = AuthMethod,
            data = ConnectFrame
        },
        subscriber_id = SubscriberId,
    ...
    case vmq_plugin:all_till_ok(on_auth_m5, [UserName, SubscriberId, Props]) of
        ...
        {ok, #{
            ...
            Frame = #mqtt5_auth{
                reason_code = ?M5_CONTINUE_AUTHENTICATION,
    ...
    case unsubscribe(User, SubscriberId, Topics, Props0, OnSuccess) of
        {ok, Props1} ->
            ...
            Frame = #mqtt5_unsuback{
                message_id = MessageId, reason_codes = ReasonCodes, properties = Props1
            },
...
connected(
    #mqtt5_auth{
        properties = #{p_authentication_method := AuthMethod} = Props,
        reason_code = RC
    },
    #state{
...
connected(
    #mqtt5_auth{
        properties = #{p_authentication_method := GotAuthMethod},
        reason_code = RC
    },
...
terminate(
    ...
    _ =
        case SessionExpiryInterval of
            0 -> ok;
            _ -> handle_waiting_acks_and_msgs(State)
        end,
...
maybe_apply_topic_alias_in(
    ...
    #vmq_msg{
        routing_key = [],
        properties = #{p_topic_alias := AliasId}
    } = Msg,
    Fun,
    ...
    case auth_on_publish(User, SubscriberId, remove_property(p_topic_alias, Msg), Fun) of
        {ok, #vmq_msg{routing_key = MaybeChangedTopic} = NewMsg, SessCtrl} ->
            ...
            {ok, NewMsg, SessCtrl, State#state{
                topic_aliases_in = TA#{AliasId => MaybeChangedTopic}
            }};
        {error, _E} = E ->
            E
    end;
...
maybe_apply_topic_alias_out(
    Topic,
    Properties,
    #state{
        topic_aliases_out = TA,
        topic_alias_max_out = Max
    } = State
) ->
    case maps:get(Topic, TA, not_found) of
        not_found ->
            case maps:size(TA) of
                S when S < Max ->
                    Alias = S + 1,
                    {Topic, Properties#{p_topic_alias => Alias}, State#state{
                        topic_aliases_out = TA#{Topic => Alias}
                    }};
                _ ->
    ...
    case vmq_plugin:all_till_ok(auth_on_register_m5, HookArgs) of
        ...
        {ok, Args0} ->
            ...
            ChangedState = State#state{
                ...
                fc_receive_max_client = min(
                    State#state.fc_receive_max_client,
                    ?state_val(fc_receive_max_client, Args, State)
                ),
...
-spec publish(module(), username(), subscriber_id(), msg(), state()) ->
    {ok, msg(), session_ctrl(), state()}
    | {error,
        atom()
        | {reason_code_name(), properties()}}.
publish(RegView, User, {_, ClientId} = SubscriberId, Msg, State) ->
    maybe_apply_topic_alias_in(
        User,
        SubscriberId,
        Msg,
...
-spec on_publish_hook(
    {ok, {integer(), integer()}} | {error, _}, list(), subscriber_id(), msg()
) -> ok | {error, _}.
on_publish_hook({ok, {0, 0}}, HookParams, SubscriberId, Msg) ->
    ...
    _ = vmq_plugin:all(on_undeliverable_publish, [
        SubscriberId, QoS, IsRetain, #matched_acl{name = AclName}
    ]),
    ok;
on_publish_hook({ok, _NumMatched}, HookParams, _SubscriberId, _Msg) ->
    _ = vmq_plugin:all(on_publish_m5, HookParams),
    ok;
on_publish_hook(Other, _, _SubscriberId, _Msg) ->
    Other.
...
dispatch_publish_(1, MessageId, Msg, State) ->
    CntOrErr = fc_incr_cnt(
        State#state.fc_receive_cnt, State#state.fc_receive_max_broker, dispatch_1
    ),
    dispatch_publish_qos1(MessageId, Msg, CntOrErr, State);
dispatch_publish_(2, MessageId, Msg, State) ->
    CntOrErr = fc_incr_cnt(
        State#state.fc_receive_cnt, State#state.fc_receive_max_broker, dispatch_2
    ),
...
dispatch_publish_qos1(MessageId, Msg, _Cnt, State) ->
    ...
    case publish(RegView, User, SubscriberId, Msg, State) of
        ...
        {error, not_allowed} ->
            ...
            Frame = #mqtt5_puback{
                message_id = MessageId, reason_code = ?M5_NOT_AUTHORIZED, properties = #{}
            },
        ...
        not_found ->
            case publish(RegView, User, SubscriberId, Msg, State) of
                ...
                {error, _Reason} ->
                    ...
                    Frame = #mqtt5_pubrec{
                        message_id = MessageId,
                        reason_code = ?M5_UNSPECIFIED_ERROR,
                        properties = #{}
                    },
                    [serialise_frame(Frame)]
            end;
...
prepare_frame(#deliver{qos = QoS, msg_id = MsgId, msg = Msg}, State0) ->
    ...
    Frame = serialise_frame(#mqtt5_publish{
...
schedule_last_will_msg(#state{will_msg = undefined}) ->
    ok;
schedule_last_will_msg(#state{
...
queue_opts_from_properties(Properties) ->
    maps:fold(
        fun
            (p_session_expiry_interval, Val, Acc) ->
                Acc#{
...
set_max_outgoing_msg_size(Max) when
    (Max == undefined) or ((Max > 0) and (Max =< ?MAX_PACKET_SIZE))
->
...
update_expiry_interval(Properties, undefined) ->
    Properties;
...
maybe_get_topic_alias_max(#{p_topic_alias_max := Max}, ConfigMax) when Max > 0 ->
    min(Max, ConfigMax);
maybe_get_topic_alias_max(_, ConfigMax) ->
    ConfigMax.

maybe_set_receive_maximum(Props, ?FC_RECEIVE_MAX) ->
    Props;
maybe_set_receive_maximum(Props, ConfigMax) when
    (ConfigMax > 0) and (ConfigMax < ?FC_RECEIVE_MAX)
->
    Props#{p_receive_max => ConfigMax}.

maybe_get_receive_maximum(#{p_receive_max := Max}, ConfigMax) when
    (ConfigMax > 0) and (ConfigMax =< ?FC_RECEIVE_MAX)
->
    min(Max, ConfigMax);
maybe_get_receive_maximum(_, ConfigMax) when
    (ConfigMax > 0) and (ConfigMax =< ?FC_RECEIVE_MAX)
->
    ConfigMax.

maybe_get_maximum_packet_size(#{p_max_packet_size := Max}, undefined) when
    (Max > 0)
->
    Max;
maybe_get_maximum_packet_size(#{p_max_packet_size := Max}, ConfigMax) when
    (Max > 0) and (ConfigMax > 0) and (ConfigMax =< ?MAX_PACKET_SIZE)
->
    min(Max, ConfigMax);
maybe_get_maximum_packet_size(_, undefined) ->
    undefined;
maybe_get_maximum_packet_size(_, ConfigMax) when
    (ConfigMax > 0) and (ConfigMax =< ?MAX_PACKET_SIZE)
->
    ConfigMax.

maybe_get_request_problem_information(#{p_request_problem_info := RequestProblemInfo}, _) ->
    unflag(RequestProblemInfo);
maybe_get_request_problem_information(_, RequestProblemInfo) ->
    RequestProblemInfo.

fc_incr_cnt(ConfigMax, ConfigMax, What) ->
    fc_log(incr, What, {ConfigMax, ConfigMax}),
    error;
fc_incr_cnt(Cnt, ConfigMax, What) when Cnt < ConfigMax ->
    fc_log(incr, What, {Cnt, ConfigMax}),
    Cnt + 1.

fc_decr_cnt(0, What) ->
    fc_log(decr, What, 0),
    0;
fc_decr_cnt(Cnt, What) when Cnt > 0 ->
    fc_log(decr, What, Cnt - 1),
    Cnt - 1.
