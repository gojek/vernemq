...
vmq_listener_start_cmd() ->
    ...
    FlagSpecs = [
        ...
        {proxy_protocol_use_cn_as_username, [
            {longname, "proxy_protocol_use_cn_as_username"},
            {typecast, fun
                ("true") -> true;
                (_) -> false
            end}
        ...
        {cafile, [
            ...
            {typecast, fun(FileName) ->
                case filelib:is_file(FileName) of
                    true -> FileName;
                    false -> {error, {invalid_flag_value, {cafile, FileName}}}
                end
            end}
        ]},
        {certfile, [
            ...
            {typecast, fun(FileName) ->
                case filelib:is_file(FileName) of
                    true -> FileName;
                    false -> {error, {invalid_flag_value, {certfile, FileName}}}
                end
            end}
        ]},
        {keyfile, [
            ...
            {typecast, fun(FileName) ->
                case filelib:is_file(FileName) of
                    true -> FileName;
                    false -> {error, {invalid_flag_value, {keyfile, FileName}}}
                end
            end}
        ]},
        {ciphers, [
        ...
        {crlfile, [
            ...
            {typecast, fun(FileName) ->
                case filelib:is_file(FileName) of
                    true -> FileName;
                    false -> {error, {invalid_flag_value, {crlfile, FileName}}}
                end
            end}
        ]},
    ...
    Callback =
        fun
            (_, [{address, Addr}, {port, Port}], Flags) ->
