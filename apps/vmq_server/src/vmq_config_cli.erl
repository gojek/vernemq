...
register_config_() ->
    ...
    ok = clique:register_config(["direct_message_passing"], fun direct_message_passing_callback/2),
    ok = clique:register_config_whitelist(["direct_message_passing" | ConfigKeys]).
    ...
    case Val of
        ...
        false ->
            ...
            case supervisor:start_child(vmq_server_sup, Spec) of
                {ok, _} -> ok;
                {error, already_present} -> supervisor:restart_child(vmq_server_sup, vmq_redis_queue_sup);
                _ -> ok
            end
    end,
