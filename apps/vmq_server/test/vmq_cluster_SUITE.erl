...
distributed_subscribe_test(Config) ->
    ...
    Sockets =
        [begin
             Connect =
                 packet:gen_connect("connect-" ++ integer_to_list(Port),
...
aborted_queue_migration_test(Config) ->
    ...
    Connect =
        packet:gen_connect("connect-aborter", [{clean_session, false}, {keepalive, 60}]),
    ...
    ok =
        vmq_cluster_test_utils:wait_until(fun() ->
                                             {0, 0, 0, 1, 10}
                                             == rpc:call(Node, vmq_queue_sup_sup, summary, [])
                                          end,
                                          60,
                                          500),
...
cluster_self_leave_subscriber_reaper_test(Config) ->
    ...
    [PubSocket | _] =
        _Sockets =
            [begin
                 Connect =
                     packet:gen_connect("connect-unclean-" ++ integer_to_list(I),
    ...
    _CleanSockets =
        [begin
             Connect =
                 packet:gen_connect("connect-clean-" ++ integer_to_list(I),
                                    [{clean_session, true}, {keepalive, 60}]),
             Connack = packet:gen_connack(0),
             Subscribe = packet:gen_subscribe(123, Topic, 1),
             Suback = packet:gen_suback(123, 1),
    ...
    ok =
        wait_until_converged(RestNodesWithPorts,
                             fun(N) ->
                                lists:usort(
                                    rpc:call(N, vmq_cluster_mon, nodes, []))
                             end,
...
cluster_dead_node_subscriber_reaper_test(Config) ->
    ...
    ok =
        wait_until_converged_fold(fun(N, {AccQ, _AccM}) ->
                                     {_, _, _, Queues, _Messages} =
                                         rpc:call(N, vmq_queue_sup_sup, summary, []),
                                     {AccQ + Queues, 0}
                                  end,
                                  {0, 0},
...
shared_subs_random_policy_online_first_test_with_local_caching(Config) ->
    shared_subs_random_policy_online_first_test([{cache_shared_subscriptions_locally, true}
                                                 | Config]).
...
shared_subs_random_policy_all_offline_test_with_local_caching(Config) ->
    shared_subs_random_policy_all_offline_test([{cache_shared_subscriptions_locally, true}
                                                | Config]).
...
routing_table_survives_node_restart(Config) ->
    ...
    Connect =
        packet:gen_connect("restart-node-test-publisher",
...
cross_node_publish_subscribe(Config) ->
    ...
    Connect = packet:gen_connect("publisher", [{keepalive, 60}, {clean_session, true}]),
    Connack = packet:gen_connack(0),
    ...
    lists:foreach(fun(N) ->
                     {ok, _} =
                         rpc:call(N,
                                  vmq_server_cmd,
                                  set_config,
                                  [shared_subscription_policy, Policy])
                  end,
                  Nodes).

set_shared_subs_local_caching(Config) ->
    Value =
        case lists:keyfind(cache_shared_subscriptions_locally, 1, Config) of
            {_, V} ->
                V;
            false ->
                false
    ...
    lists:foreach(fun(N) ->
                     {ok, _} =
                         rpc:call(N,
                                  vmq_server_cmd,
                                  set_config,
                                  [cache_shared_subscriptions_locally, Value])
                  end,
                  Nodes).
...
spawn_receivers(ReceiverSockets) ->
    Master = self(),
    lists:map(fun(S) -> spawn_link(fun() -> recv_and_forward_msg(S, Master, <<>>) end) end,
              ReceiverSockets).
...
receive_msgs(Payloads, Wait) ->
    ...
    after Wait ->
        ST = ?stacktrace,
        throw({wait_for_messages_timeout, Payloads, ST})
    end.
...
receive_nothing(Wait) ->
    receive
        X ->
            ST = ?stacktrace,
            throw({received_unexpected_msgs, X, ST})
    after Wait ->
        ok
    end.
...
recv_and_forward_msg(Socket, Dest, Rest) ->
    case recv_all(Socket, Rest) of
        {ok, Frames, Rest} ->
            lists:foreach(fun(#mqtt_publish{message_id = MsgId} = Msg) ->
...
publish_random(Nodes, N, Topic, Acc) ->
    Connect =
        packet:gen_connect("connect-unclean-pub", [{clean_session, true}, {keepalive, 10}]),
    Connack = packet:gen_connack(0),
    Payload =
        vmq_test_utils:rand_bytes(
            rand:uniform(50)),
...
%%% Hooks
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hook_uname_password_success(_, _, _, _, _) ->
    {ok, [{max_inflight_messages, 1}]}.

hook_auth_on_publish(_, _, _, _, _, _) ->
    ok.

hook_auth_on_subscribe(_, _, _) ->
    ok.
...
check_unique_client(ClientId, Nodes) ->
    Res = lists:foldl(fun({_Peer, Node, _Port}, Acc) ->
