...
%% API functions
-export([install/1, squery/2, equery/3, execute/2, ensure_pool/4]).
...
equery(PoolName, Stmt, Params) ->
    poolboy:transaction(PoolName,
                        fun(Worker) ->
                           vmq_diversity_worker_wrapper:apply(Worker,
                                                              epgsql,
                                                              equery,
                                                              [Stmt, Params])
                        end).
...
execute(As, St) ->
    case As of
        [BPoolId, BQuery | Args] when is_binary(BPoolId) and is_binary(BQuery) ->
            PoolId =
        ...
        [Config0 | _] ->
            case luerl:decode(Config0, St) of
                Config when is_list(Config) ->
                    ...
                    PoolId =
                        vmq_diversity_utils:atom(
                            maps:get(<<"pool_id">>, Options, DefaultPoolId)),
                    ...
                    User =
                        vmq_diversity_utils:str(
                            maps:get(<<"user">>, Options, proplists:get_value(user, DefaultConf))),
                    Password =
                        vmq_diversity_utils:str(
                            maps:get(<<"password">>,
                                     Options,
                    ...
                    Host =
                        vmq_diversity_utils:str(
                            maps:get(<<"host">>, Options, proplists:get_value(host, DefaultConf))),
                    Port =
                        vmq_diversity_utils:int(
                            maps:get(<<"port">>, Options, proplists:get_value(port, DefaultConf))),
                    Database =
                        vmq_diversity_utils:ustr(
                            maps:get(<<"database">>,
                                     Options,
                    ...
                    SslOpts =
                        case Ssl of
                            true ->
                                ...
                                CustomizeHostnameCheck0 =
                                    vmq_diversity_utils:atom(
                                        maps:get(<<"customize_hostname_check">>,
                                                 Options,
                                                 proplists:get_value(customize_hostname_check,
                                                                     DefaultConf))),
                                SystemCAs =
                                    vmq_diversity_utils:atom(
                                        maps:get(<<"use_system_cas">>,
                                                 Options,
                    ...
                    NewOptions =
                        [{size, Size},
                         ...
                         {ssl, Ssl},
                         {ssl_opts, SslOpts}],
                    vmq_diversity_sup:start_all_pools([{pgsql, [{id, PoolId}, {opts, NewOptions}]}],
                                                      []),
