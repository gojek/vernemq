...
-spec auth_on_register(peer(), subscriber_id(), username(), password(), boolean(), session_id()) ->
...
on_register(Peer, SubscriberId, UserName, #{?P_USER_PROPERTY := UserProps}, SessionId) ->
    {PPeer, Port} = peer(Peer),
    {MP, ClientId} = subscriber_id(SubscriberId),
    all(on_register, [
        {addr, PPeer},
...
on_register(Peer, SubscriberId, UserName, _, SessionId) ->
    {PPeer, Port} = peer(Peer),
    {MP, ClientId} = subscriber_id(SubscriberId),
    all(on_register, [
        {addr, PPeer},
...
-spec on_deliver(
    username(), subscriber_id(), qos(), topic(), payload(), flag(), _, _, session_id()
) ->
...
-spec on_client_offline(subscriber_id(), _, username(), session_id()) -> 'next'.
...
-spec on_client_gone(subscriber_id(), _, username(), session_id()) -> 'next'.
...
all_till_ok(HookName, Args) ->
    case ets:lookup(?TBL, HookName) of
        [] -> next;
        [{_, Endpoints}] -> all_till_ok(Endpoints, HookName, Args)
    end.

-spec all_till_ok(list(any()), hook_name(), any()) ->
    ok
    | {ok, any()}
    | {error, any()}.
...
convert_subscriber_id(Modifiers) ->
    case {lists:keyfind(client_id, 1, Modifiers), lists:keyfind(mountpoint, 1, Modifiers)} of
        {{_, ClientId}, {_, Mountpoint}} when is_binary(ClientId) and is_binary(Mountpoint) ->
            ...
            [
                SubscriberId
                | lists:keydelete(mountpoint, 1, lists:keydelete(client_id, 1, Modifiers))
            ];
        _ ->
            Modifiers
    end.
...
-spec call_endpoint(
    ...
    | {'hackney_url', atom(), atom(), binary(), 'undefined' | binary(),
        'nil' | 'undefined' | binary(), binary(), binary(), string(),
        'undefined'
        | integer(),
        binary(), binary()},
    map(),
...
call_endpoint(Endpoint, EOpts, Hook, Args0) ->
    ...
    Args2 = [
        {timestamp,
            list_to_binary(
                calendar:system_time_to_rfc3339(erlang:system_time(nanosecond), [
                    {unit, nanosecond}, {time_designator, $T}, {offset, "Z"}
                ])
            )}
        | Args1
    ],
...
-spec handle_response(
    hook_name(), 'false' | 'null' | 'true' | binary() | [any()] | number() | map(), map()
) -> any().
...
normalize_modifiers(Hook, Mods, Opts) when
    Hook =:= auth_on_register_m5;
...
normalize_modifiers(auth_on_subscribe, Topics, _) ->
    lists:map(
        fun(PL) ->
            {proplists:get_value(<<"topic">>, PL), proplists:get_value(<<"qos">>, PL)}
        end,
        Topics
    );
...
normalize_property(?P_USER_PROPERTY, Values, Opts) ->
    NValues = [{maybe_b64decode(K, Opts), maybe_b64decode(V, Opts)} || [{_, K}, {_, V}] <- Values],
